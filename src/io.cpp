...
/* DEPRECATED */
void
WriteCSVResults(std::ostream& out, size_t sketch_size, double preprocessing_time, double query_time, std::vector<double>& estimates) {
...
bool ReadData(std::istream& in, std::vector<std::vector<double>>& vec, size_t skip) {
	...
	while (std::getline(in, line)) {
		...
		if (i >= skip) {
			// assume data is space-delimited for now
			double entry = 0;
			while (buffer >> entry) {
				ret = true;
				row.push_back(entry);
