...
void RACE::add(const int* hashes) {
	/* 
...
void RACE::subtract(const int* hashes) {
	/*
	Input: A set of R integer hash values, one hash value for each ACE repetition
	*/
#pragma omp parallel for
...
double RACE::query(const int* hashes) {
...
std::vector<double> RACE::queryVector(const int* hashes) {
	std::vector<double> counts;
	for (size_t r = 0; r < _R; r++) {
		size_t index = hashes[r] % _range;
		counts.push_back(_sketch[r * _range + index]);
	}
	return counts;
}
...
void RACE::deserialize(std::istream& in) {
  ...
  */
	...
	_R = R;
	_range = range;
	auto* recovered_sketch = new uint32_t[_R * _range];
